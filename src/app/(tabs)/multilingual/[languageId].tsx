...
export default function MultilingualCourseScreen() {
  ...
  const quizTarget = activeLesson?.units[0];
  const quizChoices = useMemo(() => activeLesson?.units.slice(0, Math.min(3, activeLesson.units.length)) ?? [], [activeLesson]);
    ...
    return (
      <SafeAreaView style={styles.safe}><View style={styles.lessonPage}>
        ...
        {isQuiz ? <View style={styles.lessonCenter}>
          ...
          <Pressable onPress={() => quizTarget && speak(`${quizTarget.romanization}. ${quizTarget.soundHint}`, true)} style={[styles.soundButton, { backgroundColor: course.color }]}><Text style={[styles.soundButtonText, { color: course.accentColor }]}>♪ Hear it again</Text></Pressable>
          <View style={styles.choiceRow}>{quizChoices.map((choice) => <Pressable key={choice.id} onPress={() => chooseAnswer(choice.id)} style={[styles.choice, answerState === 'correct' && choice.id === quizTarget?.id && { borderColor: course.accentColor, backgroundColor: course.color }]}><Text style={styles.choiceText}>{choice.symbol}</Text></Pressable>)}</View>
  ...
  return (
    <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <View style={[styles.hero, { backgroundColor: course.color }]}><Text style={[styles.heroScript, { color: course.accentColor }]}>{course.preview}</Text><Text style={styles.heroTitle}>{course.name}</Text><Text style={styles.heroNative}>{course.nativeName}</Text><Text style={styles.heroDescription}>{course.description}</Text><View style={styles.heroMeta}><Text style={styles.heroMetaText}>{course.scriptName}</Text><Text style={styles.heroMetaDot}>•</Text><Text style={styles.heroMetaText}>{course.lessons.length} starter lessons</Text></View></View>
      <View style={styles.sectionHeader}><Text style={styles.sectionTitle}>{copy.scriptFoundations}</Text><Pressable onPress={() => setMode('worksheet')}><Text style={[styles.link, { color: course.accentColor }]}>{copy.worksheet} →</Text></Pressable></View>
      <View style={styles.lessonList}>{course.lessons.map((lesson, index) => { const completion = completions[lesson.id]; return <Pressable key={lesson.id} onPress={() => beginLesson(lesson)} style={({ pressed }) => [styles.lessonCard, pressed && styles.pressed]}><View style={[styles.lessonNumber, { backgroundColor: course.color }]}><Text style={[styles.lessonNumberText, { color: course.accentColor }]}>{completion ? '✓' : index + 1}</Text></View><View style={styles.lessonCopy}><Text style={[styles.lessonStatus, { color: course.accentColor }]}>{completion ? `COMPLETED · REPEAT ${completion.completion_count > 1 ? `×${completion.completion_count}` : ''}` : `${lesson.units.length} SYMBOLS`}</Text><Text style={styles.lessonTitle}>{lesson.title}</Text><Text style={styles.lessonDescription}>{lesson.description}</Text></View><Text style={[styles.lessonArrow, { color: course.accentColor }]}>→</Text></Pressable>; })}</View>
