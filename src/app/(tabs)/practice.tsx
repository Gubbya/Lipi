...
function allUnitsForLanguage(languageId: string): PracticeUnit[] {
  if (languageId === 'en') {
...
export default function PracticeScreen() {
  ...
  const loadPractice = useCallback(async () => {
    ...
    const available = selected.flatMap((language) => allUnitsForLanguage(language.language_id));
  ...
  useFocusEffect(useCallback(() => {
    loadPractice();
    return () => { Speech.stop(); recordingPlayer.pause(); };
  }, [loadPractice, recordingPlayer]));
  ...
  function hearTarget() {
    if (!pronunciationUnit) return;
    Speech.stop();
    Speech.speak(pronunciationUnit.symbol, { language: pronunciationUnit.locale, rate: 0.58 });
  }
